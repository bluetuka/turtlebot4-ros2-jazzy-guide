  ...
  is_docked_(false)
{
  ...
  this->declare_parameter("manage_sensors", true);
  manage_sensors_ = this->get_parameter("manage_sensors").as_bool();
  ...
  const irobot_create_msgs::msg::DockStatus::SharedPtr dock_status_msg)
{
  ...
  if (dock_status_msg->is_docked != is_docked_ && power_saver_ && manage_sensors_) {
...
{
  if (!manage_sensors_) {
    RCLCPP_DEBUG(this->get_logger(), "RPLIDAR start skipped; sensor power management disabled");
    return;
  }
...
{
  if (!manage_sensors_) {
    RCLCPP_DEBUG(this->get_logger(), "RPLIDAR stop skipped; sensor power management disabled");
    return;
  }
...
{
  if (!manage_sensors_) {
    RCLCPP_DEBUG(this->get_logger(), "OAK-D start skipped; sensor power management disabled");
    return;
  }
...
{
  if (!manage_sensors_) {
    RCLCPP_DEBUG(this->get_logger(), "OAK-D stop skipped; sensor power management disabled");
    return;
  }
